...
- 동기화 시작
- 동기화 중지  
  => Props와 state에 의존하는 Effect의 경우 여러 번 발생할 수 있음.

유의할 점은 Effect를 컴포넌트 관점에서 생각을 해본다면, "렌더링 후" 또는 "마운트 해제 전"과 같이 특정 시점에서 실행되는 "콜백" 또는 "생명주기 이벤트"로 생각하기 쉬웠는데 이런 생각은 비추.
=> Effect는 컴포넌트의 관점(마운트, 업데이트, 마운트 해제)이 아니라 각 개별 Effect의 관점(동기화 시작 및 중지 방법)에서 생각해야 한다.
...
렌더링 중 일부 하위 구성요소에서 오류가 발생하면 React가 이를 호출한다.

매개변수

- `error` 발생한 오류.
- `info` 오류에 대한 추가 정보가 포함된 개체
